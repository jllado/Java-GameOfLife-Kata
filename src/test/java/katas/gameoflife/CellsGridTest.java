...
public class CellsGridTest extends TestCase {
    ...
    public void testGetCellsLines() throws Exception {
        ...
        cellsGrid.reviveCell(new CellPosition(0, 0));
        cellsGrid.reviveCell(new CellPosition(2, 0));

        List<CellsLine> cellsLines = cellsGrid.getCellsLines();

        assertThat(cellsLines, hasSize(3));
        assertThat(cellsLines.get(0).size(), is(3));
        assertThat(cellsLines.get(0).getCells().get(0), is(new Cell(true, new CellPosition(0, 0))));
        assertThat(cellsLines.get(0).getCells().get(1), is(new Cell(false, new CellPosition(1, 0))));
        assertThat(cellsLines.get(0).getCells().get(2), is(new Cell(true, new CellPosition(2, 0))));
